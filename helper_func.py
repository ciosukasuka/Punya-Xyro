...
from config import ADMINS, FORCE_SUB_CHANNEL_1, FORCE_SUB_CHANNEL_2, FORCE_SUB_CHANNEL_3, FORCE_SUB_CHANNEL_4, FORCE_SUB_CHANNEL_5, FORCE_SUB_CHANNEL_6
...
async def subschannel(filter, client, update):
    ...
    try:
        member = await client.get_chat_member(
            chat_id=FORCE_SUB_CHANNEL_4, user_id=user_id
        )
    except UserNotParticipant:
        return False
...
async def subschannel(filter, client, update):
    ...
    try:
        member = await client.get_chat_member(
            chat_id=FORCE_SUB_CHANNEL_5, user_id=user_id
        )
    except UserNotParticipant:
        return False
...
async def subschannel(filter, client, update):
    ...
    try:
        member = await client.get_chat_member(
            chat_id=FORCE_SUB_CHANNEL_6, user_id=user_id
        )
    except UserNotParticipant:
        return False
